...
class RobotEngineeringLinkageApp:
    ...
    def update_geometry_limits(self):
        try:
            ...
            if self.dead_points:
                if current_angle < self.dead_points[0]:
                    self.angle_var.set(self.dead_points[0])
                elif current_angle > self.dead_points[1]:
                    self.angle_var.set(self.dead_points[1])
    ...
    def render_plot(self):
        try:
            ...
            if self.dead_points:
                if angle_deg < self.dead_points[0]:
                    angle_deg = self.dead_points[0]
                    self.angle_var.set(angle_deg)
                elif angle_deg > self.dead_points[1]:
                    angle_deg = self.dead_points[1]
                    self.angle_var.set(angle_deg)
            ...
            valid_config = dist_BD > 1e-9 and dist_BD <= (l3 + l4) and dist_BD >= abs(l3 - l4)
            if valid_config:
                a = (l3**2 - l4**2 + dist_BD**2) / (2 * dist_BD)
                h_sq = max(0.0, l3**2 - a**2)
                h = np.sqrt(h_sq)
                ex = (Dx - Bx) / dist_BD
                ey = (Dy - By) / dist_BD
                Px = Bx + a * ex
                ...
                perp_x = -ey
                perp_y = ex
                sign = 1 if self.elbow_var.get() == "Up" else -1
                Cx = Px + sign * h * perp_x
                Cy = Py + sign * h * perp_y
